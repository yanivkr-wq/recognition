      ...
      <header className="px-4 pt-10 pb-3 bg-card border-b border-rule shadow-hairline">
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
          ...
          <a href={avatarHref} className="flex items-center gap-2 min-w-0 justify-self-start group">
            <Avatar name={kidName} color={kidColor} avatarKey={avatarKey} size={36} />
            <span className="text-lg font-bold text-ink truncate group-hover:underline underline-offset-4">
              {kidName}
            </span>
          </a>
          ...
          <div className="flex items-center gap-2 justify-self-end">
            ...
            <form action={logoutUrl} method="POST">
              <button
                type="submit"
                aria-label={t.home.switchUser}
                className="w-9 h-9 rounded-full bg-card border border-rule flex items-center justify-center text-ink-soft hover:border-pink-pale hover:text-ink transition"
              >
