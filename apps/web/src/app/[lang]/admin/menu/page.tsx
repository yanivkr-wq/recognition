/**
 * Admin menu ("More") — the full grouped directory of every admin section.
 *
 * This is the old console grid; with the app-style bottom nav now carrying the
 * day-to-day tabs (Insights / Approvals / Players / Rewards), everything else
 * lives here behind the "More" tab. Grouped by how an admin works:
 *   • Overview — insights + the two action queues.
 *   • Players — who plays + messaging them.
 *   • Content — the things you author.
 *   • System — the record + the inbox.
 *
 * Monochrome line icons (currentColor) per BRANDBOOK §4. Server component.
 */
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {g.items.map((item) => (
                <Link
                  ...
                  className="group flex items-center gap-3 bg-card rounded-2xl shadow-card border border-rule p-4 hover:-translate-y-px hover:border-ink-faded transition"
                >
                  <span
...
/** Monochrome 22px line icons (currentColor), admin-only. */
function NavIcon({ name }: { name: IconName }) {
  ...
  switch (name) {
    case 'insights':
      return (<svg {...common}><path d="M4 19V5M4 19h16M8 16v-4M12 16V8M16 16v-6" /></svg>);
    case 'approvals':
      return (<svg {...common}><rect x="4" y="4" width="16" height="16" rx="3" /><path d="M8.5 12.5l2.5 2.5 4.5-5" /></svg>);
    case 'redemptions':
      return (<svg {...common}><rect x="4" y="9" width="16" height="11" rx="1.5" /><path d="M3 9h18M12 9v11" /><path d="M12 9c0-2.2-1.8-3.5-3-2.4S10.2 9 12 9zM12 9c0-2.2 1.8-3.5 3-2.4S13.8 9 12 9z" /></svg>);
    case 'players':
      return (<svg {...common}><circle cx="8.5" cy="8" r="3" /><circle cx="16.5" cy="9.5" r="2.4" /><path d="M3.5 19c0-2.8 2.2-5 5-5s5 2.2 5 5M14.5 19c0-2 .8-3.6 2-4.4" /></svg>);
    case 'messages':
      return (<svg {...common}><path d="M5 5h14a1.5 1.5 0 0 1 1.5 1.5v8A1.5 1.5 0 0 1 19 16H9l-4 3v-3a1.5 1.5 0 0 1-1.5-1.5v-8A1.5 1.5 0 0 1 5 5Z" /></svg>);
    case 'tasks':
      return (<svg {...common}><rect x="5" y="4" width="14" height="17" rx="2" /><path d="M9 3.5h6v2.5H9zM8.5 11l1.5 1.5 2.5-3M8.5 16l1.5 1.5 2.5-3" /></svg>);
    case 'rewards':
      return (<svg {...common}><path d="M4 7.5 11 4l9 3.2v4.3c0 4.5-3.5 7-9 8.5-5.5-1.5-7-4-7-8.5z" /><path d="M9.5 11.5l1.7 1.7 3.3-3.7" /></svg>);
    case 'campaigns':
      return (<svg {...common}><path d="M6 21V4M6 5h11l-2 3 2 3H6" /></svg>);
    case 'badges':
      return (<svg {...common}><circle cx="12" cy="9" r="5" /><path d="M9 13.5 7.5 21l4.5-2.4L16.5 21 15 13.5" /></svg>);
    case 'audit':
      return (<svg {...common}><path d="M6 3h9l4 4v14H6zM14 3v5h5" /><path d="M9 13h6M9 17h4" /></svg>);
    case 'feedback':
      return (<svg {...common}><path d="M21 11.5a8.4 8.4 0 0 1-8.5 8.5 8.5 8.5 0 0 1-3.8-.9L3 21l1.9-5.7A8.4 8.4 0 0 1 4 11.5 8.5 8.5 0 0 1 12.5 3 8.4 8.4 0 0 1 21 11.5Z" /></svg>);
  }
}
